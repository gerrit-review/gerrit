...
(function() {
  ...
  Polymer({
    ...
    ready() {
      this._permissions = this._toSortedArray(this.section.value.permissions);
    },
    ...
    _computePermissions(name, capabilities) {
      ...
      let allPermissions;
      if (name === GLOBAL_NAME) {
        allPermissions = this._toSortedArray(capabilities);
      } else {
        const labelOptions = this._computeLabelOptions();
        allPermissions = labelOptions.concat(this._toSortedArray(PERMISSIONS));
      }
